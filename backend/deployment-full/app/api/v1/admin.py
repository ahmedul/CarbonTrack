...
# =============================================================================
# CSRD ADMIN ENDPOINTS - System-wide visibility
# =============================================================================

@router.get("/csrd/reports/all")
async def get_all_csrd_reports(
    admin_user: Dict[str, Any] = Depends(require_admin),
    limit: int = 100
) -> Dict[str, Any]:
    """Get all CSRD reports across all companies (Admin only)"""
    try:
        dynamodb = boto3.resource('dynamodb', region_name='eu-central-1')
        csrd_reports_table = dynamodb.Table('carbontrack-csrd-reports-prod')
        
        response = csrd_reports_table.scan(Limit=limit)
        ...
        for item in response.get('Items', []):
            ...
            emissions = 0.0
            if 'metrics' in item and 'emissions' in item['metrics']:
                em = item['metrics']['emissions']
                emissions = float(em.get('scope_1', 0) + em.get('scope_2', 0) + em.get('scope_3', 0))
            ...
            reports.append({
                ...
                'completeness_score': float(item.get('completeness_score', 0)),
                'total_emissions_tco2e': emissions,
                'employee_count': item.get('employee_count'),
                'revenue_eur': float(item.get('annual_revenue_eur', 0)),
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to retrieve CSRD reports: {str(e)}"
        )


@router.get("/companies/csrd-users")
async def get_companies_using_csrd(
    admin_user: Dict[str, Any] = Depends(require_admin)
) -> Dict[str, Any]:
    """Get list of all companies generating CSRD reports (Admin only)"""
    try:
        dynamodb = boto3.resource('dynamodb', region_name='eu-central-1')
        csrd_reports_table = dynamodb.Table('carbontrack-csrd-reports-prod')
        
        response = csrd_reports_table.scan()
        ...
        for item in response.get('Items', []):
            ...
            companies[company_name]['user_ids'].add(item.get('user_id'))
            companies[company_name]['reports'].append({
                'year': item.get('reporting_year'),
                'status': item.get('status'),
                'created_at': item.get('created_at')
            })
            companies[company_name]['total_reports'] += 1
            companies[company_name]['years_covered'].add(item.get('reporting_year'))
        
        companies_list = []
        for company_data in companies.values():
            company_data['user_ids'] = list(company_data['user_ids'])
            company_data['years_covered'] = sorted(list(company_data['years_covered']))
            company_data['reports'].sort(key=lambda x: x.get('created_at', ''), reverse=True)
            companies_list.append(company_data)
        
        companies_list.sort(key=lambda x: x['total_reports'], reverse=True)
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to retrieve CSRD companies: {str(e)}"
        )
