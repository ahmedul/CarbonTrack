...
class CSRDDatabase:
    ...
    async def list_reports(
        ...
        for item in items:
            ...
            # Calculate total emissions from ESRS metrics
            total_emissions = 0.0
            if 'metrics' in item and item['metrics']:
                ...
                if 'emissions' in metrics:
                    ...
                    total_emissions = float(
                        emissions.get('scope_1', 0) + 
                        emissions.get('scope_2', 0) + 
                        emissions.get('scope_3', 0)
                    )
            
            # Calculate submission deadline (reporting year + 1 year + 4 months = April 30)
            submission_deadline = None
            if 'reporting_year' in item:
                from datetime import datetime
                try:
                    # CSRD reports for year N are due April 30 of year N+2
                    # E.g., 2024 data → report due April 30, 2026
                    year = int(item['reporting_year']) + 2
                    submission_deadline = datetime(year, 4, 30, 23, 59, 59)
                except:
                    pass
    ...
    async def calculate_completeness(report: CSRDReport) -> float:
        ...
        # Check company info
        company_fields = ['company_name', 'company_id', 'company_registration_number', 
                         'country', 'sector', 'employee_count', 'annual_revenue_eur']
        ...
        # Check emissions scope (from metrics)
        if report.metrics and report.metrics.emissions:
            total_fields += 4
            if report.metrics.emissions.scope_1 and report.metrics.emissions.scope_1 > 0: filled_fields += 1
            if report.metrics.emissions.scope_2 and report.metrics.emissions.scope_2 > 0: filled_fields += 1
            if report.metrics.emissions.scope_3 and report.metrics.emissions.scope_3 > 0: filled_fields += 1
            if report.metrics.emissions.total and report.metrics.emissions.total > 0: filled_fields += 1
        
        # Check ESRS metrics (20+ fields)
        if report.metrics:
            metrics_dict = report.metrics.dict()
            for key, value in metrics_dict.items():
                if key == 'emissions':  # Already counted above
                    continue
                total_fields += 1
        ...
        if report.standards_included:
            filled_fields += len(report.standards_included)
