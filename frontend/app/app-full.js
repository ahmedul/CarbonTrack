...
const app = createApp({
        ...
        return {
            ...
            apiBase: (window.API_BASE_URL && typeof window.API_BASE_URL === 'string')
                ...
                : (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1'
                    ? 'http://localhost:8000'
                    : 'https://nlkyarlri3.execute-api.eu-central-1.amazonaws.com/prod/api/v1'),
    ...
    methods: {
        ...
        async validateSession() {
            ...
            try {
                const resp = await axios.get(`${this.apiBase}/auth/me`, {
                    headers: {
                    ...
                    try {
                        ...
                        if (profileResponse.data) {
                            ...
                            this.userProfile = {
                                ...
                                email: userData.email || this.loginForm.email,
                        ...
                        console.warn('Could not fetch profile, using basic info:', profileError);
        ...
        async loadEmissions() {
            ...
            try {
                ...
                if (Array.isArray(response.data)) {
                    console.log('✅ Successfully loaded emissions from API:', response.data.length, 'entries');
                    this.emissions = response.data;
                    // Calculate totals
                    this.totalEmissions = this.emissions.reduce((sum, e) => sum + (e.co2_equivalent || e.amount || 0), 0);
                    const now = new Date();
                    this.monthlyEmissions = this.emissions
                        ...
                        .reduce((sum, e) => sum + (e.co2_equivalent || e.amount || 0), 0);
        ...
        async addEmission() {
            ...
            try {
                ...
                const response = await axios.post(`${this.apiBase}/carbon-emissions/`, emissionData, {
                    headers: {
                        'Authorization': `Bearer ${this.authToken}`,
                        'Content-Type': 'application/json'
                    }
                ...
                // API returns the emission data directly (not wrapped in {success, data})
                console.log('✅ Successfully saved emission to API', response.data);
                
                // Add the emission with the CO2 value from API
                const newEmission = {
                ...
                this.totalEmissions += (newEmission.co2_equivalent || newEmission.amount);
                this.monthlyEmissions += (newEmission.co2_equivalent || newEmission.amount);
                
                // Update goal progress
                this.goalProgress = Math.min(Math.round((this.monthlyEmissions / 300) * 100), 100);
                
                this.showNotification(`Carbon emission added successfully! CO2: ${response.data.co2_equivalent.toFixed(2)} kg`, 'success');
            } catch (error) {
                ...
                if (status === 401 || status === 403) {
                    ...
                    this.showNotification('Failed to save emission. Please try again.', 'error');
                }
            } finally {
                // Reset form
                this.emissionForm = {
        ...
        async loadPendingUsers() {
            try {
                ...
                const response = await axios.get(`${this.apiBase}/admin/pending-users`, {
                    headers: {
                        'Authorization': `Bearer ${this.authToken}`,
                        'Content-Type': 'application/json'
                    }
        ...
        async loadAllUsers() {
            try {
                ...
                const response = await axios.get(`${this.apiBase}/admin/users`, {
                    headers: {
                        'Authorization': `Bearer ${this.authToken}`,
                        'Content-Type': 'application/json'
                    }
        ...
        async loadAdminStats() {
            try {
                const response = await axios.get(`${this.apiBase}/admin/stats`, {
                    headers: {
                        'Authorization': `Bearer ${this.authToken}`,
                        'Content-Type': 'application/json'
                    }
        ...
        async approveUser(userId) {
            try {
                ...
                const response = await axios.post(
                    `${this.apiBase}/admin/users/${userId}/approve`, 
                    {},
                    {
                        headers: {
                            'Authorization': `Bearer ${this.authToken}`,
...
// Register CSRD Dashboard component
if (typeof CSRDDashboard !== 'undefined') {
    app.component('csrd-dashboard', CSRDDashboard);
    console.log('CSRD Dashboard component registered');
} else {
    console.warn('CSRD Dashboard component not loaded');
}

// Register Subscription Gate component  
if (typeof SubscriptionGate !== 'undefined') {
    app.component('subscription-gate', SubscriptionGate);
    console.log('Subscription Gate component registered');
} else {
    console.warn('Subscription Gate component not loaded');
}
